...
from .helpers import linear_to_db, unwrap_signal
...
def evm(
    ...
    * ``"blind"`` - decision-directed: the nearest ideal constellation point
    ...
    if isinstance(rx_symbols, Signal):
        ...
        return evm(resolved_symbols, ref, num_train_symbols=num_train_symbols, pmf=pmf)
    ...
    evm_db: float | ArrayType

    if evm_ratio.ndim == 0:
        if low_pwr_mask:
            return float("inf"), float("inf")
        evm_percent = float(evm_ratio * 100.0)
        # evm_ratio = sqrt(error_power) >= 0 always, so evm_ratio == 0 correctly
        # maps to -inf here (linear_to_db suppresses the divide-by-zero warning).
        evm_db = float(linear_to_db(evm_ratio, power=False))
        logger.info("EVM [%s]: %.2f%% (%.2f dB)", mode, evm_percent, evm_db)
        return evm_percent, evm_db

    evm_percent = evm_ratio * 100.0
    evm_db = linear_to_db(evm_ratio, power=False)
...
def snr(
    ...
    *,
    ...
    if isinstance(rx_symbols, Signal):
        ...
        resolved_symbols, _ = unwrap_signal(sig, field="resolved_symbols")
        return snr(resolved_symbols, ref, num_train_symbols=num_train_symbols)
    ...
    snr_db = linear_to_db(snr_linear, power=True)
...
def ber(
    ...
    *symbols*, converted to bits via ``bits_per_symbol``) leading bits are
    ...
    if isinstance(bits_rx, Signal):
        ...
        y, _ = unwrap_signal(sig, field="resolved_bits")
        r = ref
        trim = num_train_symbols or 0
